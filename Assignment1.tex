...
\end{lstlisting}
\section{Problem}
If A and B are two events such that $\Pr{(A)}=\frac{1}{4}$,$\Pr{(B)}=\frac{1}{2}$ and $\Pr{(AB)}=\frac{1}{8}$.find $\Pr{(not A and not B)}$.
\section{Solution}
...
\begin{align}
 ...
 \begin{align}
  ...
  \end{align}
  So, $\Pr{(not A and not B)}=\frac{3}{8}$\\
  \begin{figure}[h]
      \centering
      \includegraphics[width=11cm]{figure1.jpg}
  \end{figure}\\
  Here,The sky blue colored region is the required and that is $\Pr{(not A and not B)}$
\end{document}